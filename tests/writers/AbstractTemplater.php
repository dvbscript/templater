...
abstract class AbstractTemplater
{
    ...
    protected function buildPHP()
    {
        ...
        if(!file_exists($phpFile)){
            echo '<pre>'; print_r( "Создаем файл" ); echo '</pre>';
            ...
            if($tplTime > $phpTime) {
                ...
                $needToWritePhpFile = true;
                echo '<pre>'; print_r( "Шаблон \"{$this->srcFile}\" был обновлен" ); echo '</pre>';
            } else {
                echo '<pre>'; print_r( "Шаблон \"{$this->srcFile}\" не обновлялся" ); echo '</pre>';
            }
        }

        // Если основной шаблон не надо перезаписывать, всё равно заходим в инклуды - там могли быть изменения
        if(!$needToWritePhpFile) {
            echo '<pre>'; print_r( "Парсим инклюды в шаблоне \"{$this->srcFile}\"" ); echo '</pre>';
        ...
        if($needToWritePhpFile || $this->forceRecompile){
            echo '<pre>'; print_r( "Будем записывать файл на диск" ); echo '</pre>';
            $this->compile();
            $this->savePhpFile();
        }
    ...
    protected function savePhpFile()
    {
        ...
        echo '<pre>savePhpFile сохраняет файл : '; print_r( $phpFile ); echo '</pre>';
        //return true; // !!!!!!!!!!!!
